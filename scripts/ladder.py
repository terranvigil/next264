...
speed of next264 vs x264 (and optionally x265) at MATCHED preset tiers, across the
corpus, emitted as a markdown report anyone can regenerate.

For each preset in --presets it runs, per clip:
  * BD-rate (VMAF-NEG and VMAF) of next264 vs the reference at the SAME --preset,
...
Negative BD-rate = next264 spends FEWER bits at equal quality (ahead). The gate:
at matched preset, next264's BD-rate <= 0 (quality parity or better) and the speed
...
Everything is from-source and pinned: next264 = build/cli/next264, x264 =
...
# Reference encode templates. {src}/{out}/{q}/{p} are filled per run. next264 and
...
def bd_for_preset(preset, clips, args):
    """Run bdcompare.py (next264 vs ref at this preset); return {clip: (neg, v1)}."""
    ref_bin, ref_tmpl = (X264, X264_TMPL) if args.ref == "x264" else (X265, X265_TMPL)
    a = NEXT_TMPL.format(bin=shlex.quote(NEXT264), src="{src}", out="{out}", p=preset, q="{q}")
...
def speed_for_preset(preset, clip, args):
    ...
    ns = time_encode(NEXT_TMPL, NEXT264, src, preset, q, "next")
    if args.ref == "x264":
...
def main():
    ...
    for b, name in ((NEXT264, "next264"), (X264 if args.ref == "x264" else X265, args.ref)):
    ...
    lines = []
    lines.append(f"# next264 vs {args.ref}: preset-ladder BD-rate + speed")
    lines.append("")
    lines.append(f"Corpus: {', '.join(clips)}  |  {args.frames}f  |  CRF {args.points}  "
                 f"|  ref: {os.path.basename(X264 if args.ref=='x264' else X265)}")
    lines.append("BD-rate = next264 vs " + args.ref + " at the SAME --preset; "
                 "negative = next264 ahead (fewer bits at equal quality). "
                 f"Speed on {args.speed_clip}, next264/{args.ref} wall-time (>1 = next264 slower).")
    lines.append("")
    ...
    lines.append("**Reading this table:** medium (and slower) is the true "
                 ...
                 "FAST tiers are NOT tool-aligned with " + args.ref + ": next264's "
                 ...
                 "ultrafast/veryfast means next264 is spending more compute for higher "
                 ...
                 "(x264 NEON asm vs next264 NEON intrinsics); the pure-C algorithmic "
