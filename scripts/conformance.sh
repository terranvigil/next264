...
check_threading() {     # check_threading <label> <src> <feat>
    ...
    # shellcheck disable=SC2086
    Y264_STQ=0 Y264_RC_CARRY=0 Y264_DIRECT_AUTO=0 "$enc" --input-y4m "$src" --qp 26 --keyint 3 $feat --threads 1 -o "$p.1.264" 2>/dev/null || true
    # shellcheck disable=SC2086
    Y264_RC_CARRY=0 Y264_DIRECT_AUTO=0 "$enc" --input-y4m "$src" --qp 26 --keyint 3 $feat --threads 2 -o "$p.2.264" 2>/dev/null || true
    # shellcheck disable=SC2086
    Y264_RC_CARRY=0 Y264_DIRECT_AUTO=0 "$enc" --input-y4m "$src" --qp 26 --keyint 3 $feat --threads 8 -o "$p.8.264" 2>/dev/null || true
...
check_rc() {    # check_rc <label> <src> <spec>   -- recon-match + thread determinism
    ...
    t=$((t + 1))
    # shellcheck disable=SC2086
    Y264_STQ=0 Y264_RC_CARRY=0 Y264_DIRECT_AUTO=0 "$enc" --input-y4m "$src" $spec --keyint 6 --threads 1 -o "$p.1.264" 2>/dev/null || true
    # shellcheck disable=SC2086
    Y264_RC_CARRY=0 Y264_DIRECT_AUTO=0 "$enc" --input-y4m "$src" $spec --keyint 6 --threads 4 -o "$p.4.264" 2>/dev/null || true
