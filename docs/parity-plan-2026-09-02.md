...
| 8 | A3 clamps, retire CFLOOR/CGUARD; A4 29-clip ABR table; A5 flip (pre-authorized, decision 1) and `rcp_lag=1` | 1 | **A3 folded into A2** (lstep + absolute clamps are in RF2; CFLOOR/CGUARD never reach it). **Cross-GOP carry SHIPPED 09-02** ahead of A4 (owner): `rc.carry` API + `Y264_RC_CARRY` default on; RF2 stockholm -23.4 -> -11.7 (x264 -11.4), the default model tightens too; `local/records/rc-carry-2026-09-02.md`. **A4 read 09-02** (29 clips, int build): ABR RF2 vs x264 ABR medians CIF -4.1 / 720p -8.2 / 1080p -1.4 (default -0.5 / +14.7 / +8.8); tax 5.5 / 9.4 / 6.9 vs x264 3.5 / 4.4 / 3.8 (default 10 / 33 / 18). Loses to the default only at the top of the rate range (riverbed +13 pts, crowd_run +6): rate-keyed B cascade is the arm. `local/records/a4-abr-table-2026-09-02.md`. **A5 flip = owner call on this table** | todo (A5) |
...
`rf_qp = 6·log2(cplxr_sum / wanted_bits_window) + K`, seeded from a bitrate
...
Read-only study, 2026-09-02. x264 references are `encoder/ratecontrol.c` in
the local r3223 tree unless stated; ours are `src/encoder/encoder.c`.

### x264

Entry `x264_ratecontrol_start` (:1496) → `rate_estimate_qscale` (:2399) →
`qscale2qp`, clip to `[qp_min, qp_max]` (:1519), store `rc->qpm`, then
`accum_p_qp_update` (:1410).

One equation, two rate factors. `get_qscale` (:2002) is `q = rceq /
rate_factor`. CRF passes `rate_factor_constant`, computed once at :644 as
`base_cplx^(1−qcomp) / qp2qscale(crf + mbtree_offset)`. ABR passes
`wanted_bits_window / cplxr_sum` (:2581). Nothing else in the frame-QP path
...
default), :764 forces `qcompress = 1` and `pb_factor = 1`, and `get_qscale`
...
`short_term_cplxsum / count` (:2560-2563) over `last_satd` from
`x264_rc_analyse_slice` (slicetype.c:1993), the lookahead lowres cost, one
domain for I, P and B, mb-tree-recalculated and AQ-weighted.

Adaptation. `ratecontrol_end` (:1918): `cplxr_sum += bits ×
qp2qscale(qpa_rc) / last_rceq` (B divides by `pb_factor`, :1923);
`wanted_bits_window += duration × bitrate` (:1926); both `×= cbr_decay`
...
`abr_buffer = 2 × rate_tolerance × bitrate`, then `×= max(1, sqrt(time_done))`;
`overflow = clip(1 + (predicted_bits − wanted_bits) / abr_buffer, 0.5, 2)`;
...
two nearest refs' `f_qp_avg_rc`, plus `pb_offset` (half for a ref B).
`ip_offset = 6·log2(ip_factor)`, `pb_offset = 6·log2(pb_factor)` (:821), and
`pb_offset` is 0 under mb-tree.

Startup (:813-818, :836): `cplxr_sum = 0.01 × 7e5^qcomp × sqrt(mb_count)`,
`wanted_bits_window` = one frame of target bits, `accum_p_norm = 0.01`,
...
damped above `QP_MAX_SPEC`. The feedback consumes `qpa_rc`, the pre-AQ frame
average (:1602, :1842), so AQ offsets never enter `cplxr_sum`; mb-tree's
...
state (`cplxr_sum`, `wanted_bits_window`) from the most recently ended one.
...
So ABR ≈ CRF + feedback: identical `get_qscale`, identical I/B derivation,
...
cplxr_sum seed `0.01 × 7e5^0.6 × sqrt(mbs)`.
...
x264's asymmetric lstep clip. `cplxr_sum` grows only when frames cost bits,
`wanted_bits_window` grows linearly with time, so wherever the complexity
...
be spent downward, and `cplxr_sum` is fed a mixed-domain `rceq` (intra SATD
...
1. Rate factor in QP domain: `rf_qp = 6·log2(cplxr_sum / wanted_bits_window)
...
3. Adaptation law: after each coded frame, `cplxr_sum += bits ×
   qscale(qpa_pre_aq) / rceq`, `wanted_bits_window += target_bpf`, with `rceq`
...
Read-only study, 2026-09-02. x264 references are `encoder/analyse.c` and
`encoder/macroblock.c` in the local tree; ours `src/encoder/macroblock.c`.

### x264 at preset medium (subme 7, ref 3, mixed refs on)

There is no pre-search probe at medium. `x264_macroblock_analyse` sets
`analysis.b_try_skip = 1` whenever `i_subpel_refine >= 3` (analyse.c:3006-3008)
and does not call the probe. The standalone `x264_macroblock_probe_pskip`
call (analyse.c:3013) is the subme < 3 branch only, and even there gated on a
...
The post-ref-0 early exit (`mb_analyse_inter_p16x16`, analyse.c:1254; exit at
...
`i_maxref` to ref 0 + neighbour refs whenever 16x16 chose ref 0 (analyse.c:1351).
...
Read-only study, 2026-09-02. x264 references: `common/mvpred.c`,
`common/macroblock.c`, `encoder/analyse.c`, `encoder/encoder.c`; ours
`src/encoder/macroblock.c`, `src/encoder/encoder.c`.

### x264's mechanism

Entry: `x264_mb_predict_mv_direct16x16` (mvpred.c) dispatches on
...
Temporal (`mb_predict_mv_direct16x16_temporal`). refIdxL1 = 0 for the whole
...
Frame-thread MV guard (analyse.c:339-405): `mv_max_spel[1]` is recomputed
...
against the bound (mvpred.c:269), spatial tests both lists' y
(mvpred.c:388-396).

Tournament (analyse.c:3305-3420). `i_type = B_SKIP` first. If direct is
...
(analyse.c:1844) prices direct as four 8x8 `mbcmp` against the already-MC'd
...
7λ (Bi) from the same table (analyse.c:2071-2073). Direct is RD-refined in
`mb_analyse_b_rd` (analyse.c:2622-2632), reusing the prediction, only when
`i_cost16x16direct <= i_cost × 33/32`. After that RD stage B_SKIP wins if
`i_bskip_cost` beats all four 16x16 RD costs.

AUTO (encoder.c:152-171, analyse.c:3314-3334). Per B macroblock derive both
...
| # | site | x264 | yah264 |
|---|---|---|---|
| D1 | colocated block that predicted from list 1 only | `map_col_to_list0(−1) = −1` → direct unavailable for the MB (mvpred.c:271-278) | `colpoc = cur_l1poc0` (encoder.c:11346), resolves by POC, temporal vector derived from a backward field (macroblock.c:5375-5382) |
...
| D3 | mb_type rate in the SATD ranking | `i_mb_b_cost_table`: direct 1λ, L0/L1 3λ, Bi 5-7λ, B_8x8 9λ (analyse.c:1852, 2071-2073) | `bmb_cost_on()` defaults 0 (macroblock.c:6019); every B mode compared on raw SATD |
...
| D6 | thread MV guard | per-MB drop against a bound from a cond_wait on every list-0 and list-1 reference (analyse.c:353-371) | per-MB drop against the fixed `stair_mvy_max = 4·(16·stair_lag − 24)` (encoder.c:4623), list-0 gated on `stair_l0_clamp`, list-1 tested only for temporal (macroblock.c:7222-7240) |
...
(analyse.c:355-364) iterates `for i = (i_type == B); i >= 0; i--`, cond-waiting
...
offsets (x264 ratecontrol.c:1761 uses `f_qp_offset_aq` on non-ref too, no
...
`encoder/slicetype.c`, `encoder/encoder.c`, `encoder/analyse.c`; ours
...
memo check on `fenc->i_cost_est[b−p0][p1−b]` (slicetype.c:848) and returns on
...
`macroblock_tree_finish` converts to `f_qp_offset`. When mb-tree moves the
...
thread encodes the frame (`x264_rc_analyse_slice`, slicetype.c:1993). The
reference wait is row-based: analyse.c:362 calls
