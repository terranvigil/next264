...
A frame is dead as soon as `next264_encoder_encode` returns for it: the call
...
window >= 1 is correct and `N264_STREAM_WINDOW=4` is byte-identical.
`N264_STREAM_WINDOW` overrides the budget outright.
...
falls back to `N264_GOP_EVEN=1`, and therefore **moves ABR bits**: the drain
...
**The cut-aware split.** `next264_scan_idr_frames` takes `luma[0..n)` and needs
...
and the refusal that goes with it. That is a deliberate trade: `N264_CUT_SPLIT`
...
Flat. 1.559 MB per frame of clip becomes 0.004. `N264_STREAM_STAT=1` reports the
...
workers run, `N264_STREAM_STAT` reports 17 frames resident of a 208-frame window,
...
`next264_encoder_close` was leaving nine allocations behind, three of them the
...
11.93 top-core equivalents of 18 and next264 keeps 10.79 whole-input and 10.87
