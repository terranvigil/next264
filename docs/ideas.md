...
Start from the N264_BPROF buckets (B ME 10.3% of wall, RD survivors 9.5%,
...
- **Sync-lookahead ring buffering (`done`, N264_LA_BUF, default 0).**
 ...
 next264: grow the ring's CAPACITY to `la_depth + k` (`N264_LA_BUF=k`) while
 ...
 N264_LA_THREAD combined (park_joy_720p 500f single-GOP, best-of-5): still
...
- SME / SME2 for transforms and SATD (`idea`, benefit M, effort L). The M-series
 scalable matrix extension suits the 4x4/8x8 transform and Hadamard SATD. Detect
...
- CABAC entropy coding (`done`, BD-rate ~10-15%, effort L). Universal win.
 ...
 harness (scratchpad/next264_log + instrumented ffmpeg) is reusable for future
 CABAC tools. There is also a CABAC decode engine (n264_cabac_dec_*) worth
...
streams + decoded MD5s) validate a *decoder* -- aimed at the `n264_cabac_dec_*`
...
- B-frame deblocking (`done`). The bS derivation handles B's
 dual-list motion (deblock.c strength: different reference-picture set / MV
...
- **aq-mode 2**: already have it. Our AQ centres per-MB log2(var) on the frame
 mean (auto-variance). **aq-mode 3** (dark bias) hook added (N264_AQ_DARK) but
