...
**Nothing here shipped, and the conclusion is negative: every cut to next264's
motion search costs BD-rate out of proportion to the speed it buys.** The
instrumentation (`N264_ME_STATS`) is in the tree as a permanent diagnostic; the
...
cost model so next264 reaches x264-medium quality at x264-medium compute. ME
...
- **ARCH-1 "hex parity":** make next264's hex-only search reach x264 quality,
...
The asymmetry to explain: with UMH, next264 is at corpus BD parity with x264
...
early-skip detection, threshold-gated escalation everywhere. next264's
...
next264's own fallback balance, measured not copied.

### The gating signal: the lowres oracle

next264 already computes, for every MB of every frame before it is coded, a
...
**Instrumentation first (`N264_ME_STATS`, no behavior change).** Per-search
...
admit) stay intact. Env `N264_ME_GATE` bit 0.
...
`N264_ME_STATS` instrumented the real search over the 5-clip corpus (replicated
...
1. **UMH is bigger than the plan assumed: 28-37% of pure-C.** `N264_NO_UMH=1`
...
4. **Behaviour-matched partition early-termination costs BD, so G3 is dead.**
   ...
   `i_cost8x8 < i_cost16x16 + thresh`; env `N264_PART_ET`, subme<=8). Speed is
...
5. **Diamond subpel is BD-dead.** `N264_SUBPEL=1` (diamond to convergence)
...
**The unified finding, measured four ways:** next264's UMH plus full partition
...
because next264 gets less quality per ME candidate than x264 and compensates
...
medium, next264's residual and entropy chain is at COUNT PARITY with x264 (dct
