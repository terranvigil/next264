...
`xbprof.patch` fair build (`<reference-internal>=1`, vectorized C + asm per the
...
- **x264's entry skip-commit is NOT their shedder.** At mbrd their entry bound
  is `bskip_ssd <= (6*lambda2+128)>>8` (<reference-source> ~3350: skip commits when its
...
## 2. What x264's ladder actually does (<reference-source>, read 08-27)
