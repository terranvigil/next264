# yah264 comparison players

Two self-contained, dark-theme browser players for eyeballing yah264 output,
adapted from the veo project's comparison player.

- `compare.html` - side-by-side slider comparing **two encodings** of the same
  source (yah264 vs x264, or shot-based vs regular yah264, etc.). Drag the
...
```sh
SRC=src.y4m
build/cli/yah264 --input-y4m $SRC --qp 32 -o yah264.264
...
variant. Point at an explicit model with `YAH264_VMAF_MODEL=/path/to/model.json`.
