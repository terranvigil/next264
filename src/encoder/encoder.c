 ...
 * not let the curve decide this at all -- get_diff_limited_q OVERWRITES
 ...
 * x264's qscale2bits(rce, 1) with tex/mv/misc lumped -- pass 1 records one
...
/* x264's get_diff_limited_q . Forces the I and B qscales
 ...
 * luma weight per active list-0 reference (fade estimation runs per ref). */
    ...
    if (e->abr_on) {
        ...
        /* Seed the complexity->bits scale (x264 cplxr_sum init) so the FIRST frame
 ...
 * la_buf frames of latency and no bits. */
        ...
        if (e->abr_on && abr_rf2_env()) {
            if (!e->mbtree_on) {
                ...
                int nmb = e->width_in_mbs * e->height_in_mbs;
                e->cplxr_sum = 0.01 * 7.0e5 * sqrt((double)(nmb > 0 ? nmb : 1));
                e->wanted_bits_window = e->abr_target_bpf;
            }
        }
...
/* Sum-of-squared-deviations of a w x h block, x264's ac_energy_var for a plane
                             ...
                             float strength, double *invq, float *aq_off)
{
    int n = wmb * hmb;
    double sum = 0;
    /* x264's ac_energy_mb sums every plane; the chroma block is the MB's
...
/* Scene-cut decision from lowres cost sums (x264 slicetype.c): the P cost is
...
/* --- behaviour-matched anchor-pair lowres ME (slicetype.c slicetype_mb_cost).
 *
 ...
 * frequently coincide after fpel rounding + clamping. */
    ...
    /* x264 hex_search: radius-2 (fpel) hexagon iterated to convergence
 ...
 * iteration (x264 subpel_iterations[4] = {.,.,1,1}). */
...
/* Y264_ABR_RFQP=1: plan step A1 (docs/parity-plan-2026-09-02.md, work item
 ...
 * (blurred rceq, cplxr_sum, wanted_bits_window, rf, overflow, the raw,
 ...
 * decides; the other is a SHADOW: its state (cplxr_sum, wanted_bits_window,
 * the st_cplx EWMA, last_qscale_for, the P-track anchors) is fields only the
 ...
 * -- x264's target_fill goal at a one-frame horizon:
 *
 ...
 * offset instead (measurement arm). */
            ...
            double n_before = e->wanted_bits_window / e->abr_target_bpf;
            double mean_before = n_before > 0 ? e->cplxr_sum / n_before : 0.0;
            e->cplxr_sum += contrib;
            e->wanted_bits_window += e->abr_target_bpf;
            abr_track_update(e, p->fqp, p->type);
            if (abr_rfqp_trace())
                fprintf(stderr, "RFQA seq=%u type=%d bits=%.0f fqp=%d qoff=%.2f rceq=%.3f | cplxr_sum=%.0f wanted=%.0f "
                        "rfqp6=%.2f acc_p=%.2f contrib=%.0f mean_before=%.0f\n", p->seq, p->type, p->bits, p->fqp,
                        p->qoff, p->rceq, e->cplxr_sum, e->wanted_bits_window,
                        6.0 * log2(e->cplxr_sum / e->wanted_bits_window),
                        e->accum_p_norm > 0 ? e->accum_p_qp / e->accum_p_norm : 0.0, contrib, mean_before);
        }
 ...
 * plus in-flight predictions, each pending frame owing one target_bpf. */
        ...
        if (e->abr_rf2) {
            ...
            if (type != 2) {                   /* B: anchor base + frame_qp cascade, as CRF */
                ...
                double n_done = e->wanted_bits_window / e->abr_target_bpf;
                double wanted = e->wanted_bits_window + np * e->abr_target_bpf;
                double cplxr = e->cplxr_sum;
                if (np > 0 && n_done > 0) cplxr += np * (e->cplxr_sum / n_done);
 ...
 * complexity, which is x264's own stated reason. */
                ...
                if (type == 0 && e->last_nonb_type != 0 && e->abr_cum_actual > 0)
                    tr_anch = e->accum_p_qp / e->accum_p_norm;
