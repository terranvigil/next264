 ...
 */
#ifndef NEXT264_CAVLC_H
#define NEXT264_CAVLC_H
...
enum {
    N264_CAVLC_LUMA_DC   = 16,   /* Intra16x16 luma DC block */
    N264_CAVLC_LUMA_AC   = 15,   /* Intra16x16 luma AC block (positions 1..15) */
    N264_CAVLC_LUMA_4x4  = 16,   /* Intra4x4 / inter 4x4 block */
    N264_CAVLC_CHROMA_AC = 15,   /* chroma AC block */
    N264_CAVLC_CHROMA_DC = 4,    /* chroma DC block (4:2:0), coded with nC == -1 */
};
...
/* Bit length n264_cavlc_residual would emit for the same block, without a
...
int n264_cavlc_residual_len(const dctcoef *coeff, int maxNumCoeff, int nC,
                            int stride);
...
void n264_cavlc_coeff_token(int col, int total_coeff, int trailing_ones,
                            int *len, int *code);
void n264_cavlc_total_zeros(int max_num_coeff, int total_coeff, int total_zeros,
                            int *len, int *code);
void n264_cavlc_run_before(int zeros_left, int run, int *len, int *code);

#endif /* NEXT264_CAVLC_H */
