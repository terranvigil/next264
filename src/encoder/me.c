 ...
 */
#include "me.h"
#include "next264.h"            /* NEXT264_ME_* -- the --me values we gate on */
#include "../dsp/mc.h"
#include "../dsp/pixel.h"
...
{
    static int v = -2;
    if (v == -2) { const char *e = getenv("N264_NO_UMH"); v = e ? (atoi(e) ? 1 : 0) : -1; }
    return v;
}

/* Whether the UMH wide grid is eligible this frame (before the per-search
 * small-block / cheap gates). Precedence: N264_NO_UMH env > --me method >
 * subme ladder (>=8 = slow+). */
static int umh_allowed(void)
{
    ...
    if (m == NEXT264_ME_UMH) return 1;
    if (m == NEXT264_ME_DIA || m == NEXT264_ME_HEX) return 0;
...
int n264_me_hex_features(void)
{
    ...
    int m = atomic_load_explicit(&s_me_method, memory_order_relaxed);
    if (m == NEXT264_ME_UMH || m == NEXT264_ME_DIA) return 0;  /* no compensation */
    if (m == NEXT264_ME_HEX) return 1;
    return atomic_load_explicit(&s_me_subme, memory_order_relaxed) < 8;
}
 ...
 * n264_me_set_subme (before any worker thread runs ME). */
 ...
 * back to the multiply path. Primed from n264_mb_warm_statics for every value
 ...
 * N264_SUBPEL env still overrides this (see subpel_mode). Set once before any
 * worker runs ME, like n264_me_set_subme. */
...
static _Thread_local struct me_tls {
    const n264_hpel_ref_t *hpel;     /* half-pel plane registry (see me.h) */
 ...
 * search). N264_ME_ET_IMP gates it. */
...
void n264_me_set_et_off(int off) { s_met.me_et_off = off; }
void n264_me_set_et_class(int c) { s_met.et_class = c; }
void n264_me_set_stq(int q) { s_met.stq = q; }
 ...
 * position-by-position against n264_mc_luma_c's own switch. */
...
{
    if (s_hpc_band < 0) {
        const char *e = getenv("N264_HPEL_CENSUS");
...
{
    if (!n264_hpel_census_on() || !h) return;
...
__attribute__((destructor)) static void hpc_dump(void)
{
    ...
    fprintf(stderr, "=== N264_HPEL_CENSUS band=%d ===\n", s_hpc_band);
...
{
    static int v = -1;
    if (v < 0) { const char *e = getenv("N264_HPEL_LIST"); v = e ? (atoi(e) ? 1 : 0) : 1; }
    return v;
}

void n264_me_set_list(int l) { s_met.me_list = hpel_list_on() ? (l == 1) : 0; }

static int hpel_thresh_on(void)
{
    static int env = -2;
    if (env == -2) { const char *e = getenv("N264_HPEL_THRESH"); env = e ? atoi(e) : -1; }
    if (env >= 0) return env;
...
{
    static int v = -1;
    if (v < 0) { const char *e = getenv("N264_ME_STATS"); v = e ? atoi(e) : 0; }
    return v;
}
...
{
    static int v = -1;
    if (v < 0) { const char *e = getenv("N264_ME_ETSTAT"); v = e ? atoi(e) : 0; }
    return v;
}

/* N264_ME_ET=<K>: integer-search early-out. When the best SEED already costs
...
/* N264_ME_ET_SHAPE: which block shapes the early-out is allowed on.
...
/* N264_ME_ET16=<K>: a SEPARATE, independent early-out threshold for the 16x16
...
{
    static int v = -1;
    if (v < 0) { const char *e = getenv("N264_ME_ET_FT"); v = e ? atoi(e) : 7; }
    return v;
}
/* N264_ME_ET_CROSS=1: early-out MBs still probe the UMH cross (see the
 * eligibility computation at the search site). Default 0 = byte-identical. */
static int me_et_cross(void)
{
    static int v = -1;
    if (v < 0) { const char *e = getenv("N264_ME_ET_CROSS"); v = e ? atoi(e) : 0; }
    return v;
}
 ...
 * planes, reproducing n264_mc_luma exactly: half/integer positions are a strided
                            ...
                            int ix, int iy, int fx, int fy, int w, int h)
{
    ...
    if (qi & 5) {
        ...
        n264_pred_avg2(pred, 16, s1, s2, rs, w, h);
    } else {
        n264_pred_copy(pred, 16, s1, rs, w, h);
    }
}

/* Public plane-read luma MC into a stride-16 pred block. Byte-identical to
 * n264_mc_luma: reads this reference's registered half-pel planes (n264_me_set_hpel)
 ...
 * n264_mc_luma has a SIMD kernel (the plane read is scalar here too), but a real win
                     ...
                     int bx, int by, int mvx, int mvy, int w, int h)
{
    ...
    if (hp_h && ix >= -B && iy >= -B &&
        ...
        n264_mc_luma(pred, 16, ref, rs, pw, ph, bx, by, mvx, mvy, w, h);
}
...
{
    static int v = -1;
    if (v < 0) { const char *e = getenv("N264_F3"); v = e ? atoi(e) : 1; }
    return v;
}
 ...
 * of one n264_me_search call -- block position, dims, the SAD kernel resolved
...
typedef struct {
    ...
    int pw, ph, bx, by, w, h;
    n264_sad_fn sad_fn;              /* NULL -> generic sad (unsupported shape) */
    n264_sad_x4_fn sadx4_fn;         /* NULL -> per-candidate probes */
...
{
    int pu = -1;
    if (w == 16)     pu = h == 16 ? N264_PU_16x16 : (h == 8 ? N264_PU_16x8 : -1);
    else if (w == 8) pu = h == 16 ? N264_PU_8x16 : (h == 8 ? N264_PU_8x8
                         : (h == 4 ? N264_PU_8x4 : -1));
    else if (w == 4) pu = h == 8 ? N264_PU_4x8 : (h == 4 ? N264_PU_4x4 : -1);
    return pu;
}
...
{
    if (w == 16 && h == 16)
        return n264_dsp.satd16x16(a, as, b, bs);
    int s = 0, y = 0;
    for (; y + 8 <= h; y += 8) {
        int x = 0;
        for (; x + 8 <= w; x += 8)
            s += n264_dsp.satd8x8(a + y*as + x, as, b + y*bs + x, bs);
        for (; x < w; x += 4)
            for (int yy = 0; yy < 8; yy += 4)
                s += n264_dsp.satd4x4(a + (y+yy)*as + x, as, b + (y+yy)*bs + x, bs);
    }
    for (; y < h; y += 4)
        for (int x = 0; x < w; x += 4)
            s += n264_dsp.satd4x4(a + y*as + x, as, b + y*bs + x, bs);
    return s;
}
...
static int sad_blk(const pixel *a, int as, const pixel *b, int bs, int w, int h)
{
    ...
    if (pu >= 0) return n264_dsp.sad[pu](a, as, b, bs);
...
{
    static int v = -1;
    if (v < 0) { const char *e = getenv("N264_F1"); v = e ? atoi(e) : 1; }
    return v;
}
 ...
 * (fx|fy odd) still needs the 2-tap average build. */
    ...
    if (inb) {
        ...
        n264_mc_luma(pred, 16, c->ref, c->rs, c->pw, c->ph, c->bx, c->by,
                     mvx, mvy, c->w, c->h);
    }
 ...
 * coastguard -0.06%). Set N264_ME_SMALL_NOUMH=0 to restore the full grid. */
static int me_small_noumh(void)
{
    static int v = -1;
    if (v < 0) { const char *e = getenv("N264_ME_SMALL_NOUMH"); v = e ? atoi(e) : 1; }
    return v;
}
 ...
 * use umh_allowed / n264_me_hex_features above.) */
static int no_umh(void)
{
    return !umh_allowed();
}

/* ORACLE-SEED DIAGNOSTIC (N264_HEX_ORACLE, only meaningful with N264_NO_UMH=1):
...
{
    static int v = -1;
    if (v < 0) { const char *e = getenv("N264_HEX_ORACLE"); v = e ? atoi(e) : 0; }
    return v;
}
...
void n264_me_set_isb(int b) { s_met.me_isb = b; }
...
void n264_me_set_ymax(int ymax_qpel) { s_met.me_ymax = ymax_qpel; }

/* UMH search radius (integer pels). Default 16 = x264 me_range. Reducing it with
 * good seeds in place is a cheaper wide search (A/B via N264_UMH_RANGE). */
static int umh_range(void)
{
    static int v = -1;
    if (v < 0) { const char *e = getenv("N264_UMH_RANGE"); v = e ? atoi(e) : 16; }
    return v > 0 ? v : 16;
}

/* Subpel refinement pattern (see the subpel loop in n264_me_search):
 * 0 = 8-neighbour square (default), 1 = 4-point diamond, 2 = capped diamond. */
static int subpel_mode(void)
{
    /* Precedence: explicit N264_SUBPEL env (A/B override) > preset-set value >
 * default 0 (square). env is checked once (-2 = unchecked). */
    static int env = -2;
    if (env == -2) { const char *e = getenv("N264_SUBPEL"); env = e ? atoi(e) : -1; }
    if (env >= 0) return env;
 ...
 * byte-identical. N264_HPEL_SAD forces on(1)/off(0) for A/B. */
static int hpel_sad(void)
{
    static int env = -2;
    if (env == -2) { const char *e = getenv("N264_HPEL_SAD"); env = e ? atoi(e) : -1; }
    if (env >= 0) return env;
    return atomic_load_explicit(&s_me_subme, memory_order_relaxed) <= 8;
}

int n264_me_search(const pixel *src, int ss,
 ...
 * (UMH) path truncates toward -inf (& ~3); x264's <reference-internal> rounds to nearest
 ...
 * hex-only path; keep truncation on the default path (byte-identity). */
    int align_round = n264_me_hex_features();
 ...
 * safe MV rectangles, all resolved once per search instead of per probe. */
    ...
    me_ctx mc = { src, ss, ref, rs, pw, ph, bx, by, w, h,
                  pu >= 0 ? n264_dsp.sad[pu] : NULL,
                  pu >= 0 ? n264_dsp.sad_x4[pu] : NULL,
 ...
 * legal. == sad_int's old per-probe test (B only with the F3 border). */
        const int Bi = f3_border() ? N264_LUMA_BORDER : 0;
 ...
 * quarter-pel units, so one integer pel is 4. */
    ...
    /* Early-out accounting (N264_ME_ETSTAT): the seed result is the state an
 ...
 * stage is the work it would delete. */
    ...
    long etp = 0;
    /* N264_ME_ET: the seed is already good enough per pixel -- skip the integer
 ...
 * from it (hex "given the right seed"), then continue as pure hex-only. */
    ...
    /* Hex-only (N264_NO_UMH) mirrors x264 <reference-internal>'s terminal 8-point SQUARE
 ...
 * (the UMH grid already covers the diagonals). */
    ...
    int sq_refine = n264_me_hex_features();
 ...
 *
 * Pattern is env-selectable (N264_SUBPEL) for the speed/quality trade:
 ...
 * and evaluated costs are >= bcost -- exact re-probe removal. */
        ...
        /* Env-selected variants: hpel in SAD (N264_HPEL_SAD), 4-point diamond
 * (N264_SUBPEL=1) or capped diamond (=2). Each level re-scores the best
...
static void me_et_stats_dump(void)
{
    ...
    if (!tn) return;
    fprintf(stderr, "\n=== N264_ME_ETSTAT: integer-search work after the seeds ===\n");
...
void n264_me_stats_dump(void)
{
    ...
    fprintf(stderr, "\n=== N264_ME_STATS (E2, adaptive-me-design.md) ===\n");
 ...
 * n264_mb_warm_statics, which calls this. Afterwards the search threads only
