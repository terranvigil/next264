 ...
 */
#ifndef NEXT264_TRANSFORM_H
#define NEXT264_TRANSFORM_H
 ...
 * n264_sub4x4_dct calls -- the point is that horizontally adjacent blocks
...
int n264_mf4_at(int idx, int qp);
...
const int *n264_dct4_w2_row(void);
const int *n264_dct8_w2_row(void);
...
const int *n264_dct4_w2_row_zz(void);
const int *n264_dct8_w2_row_zz(void);

/* Coefficient zig-zag scan (frame), scan order -> raster index. */
extern const uint8_t n264_zigzag4[16];
...
void n264_zigzag_abs_4x4(int out[16], const dctcoef in[16]);   /* C only */
