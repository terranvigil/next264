 ...
 * unless built with -DN264_STAGE_PROF.
...
void n264_stg_count(int bucket) { g_stg_calls[bucket]++; }

__attribute__((destructor)) static void n264_stg_dump(void)
{
    if (!g_inited) return;
    double wall = stg_ms() - g_wall0, tot = 0;
    for (int i = 0; i < STG_N; i++) tot += g_stg_ms[i];
    fprintf(stderr, "\n=== N264_STAGE_PROF (attributed %.1f ms) ===\n", tot);
